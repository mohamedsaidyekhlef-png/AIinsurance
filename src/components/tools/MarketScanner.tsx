...
import { Globe, Database, Wifi, Terminal, ExternalLink, Loader2 } from 'lucide-react';
...
export const MarketScanner = () => {
  ...
  useEffect(() => {
    ...
    // Reliable Discovery (Every 3s)
    // FIX: Wrapped async logic in a synchronous function to prevent "t._onTimeout is not a function" error
      ...
      const runDiscovery = async () => {
        try {
          ...
          if (newData && newData.length > 0) {
             ...
             setLogs(prev => [...prev, `[SUCCESS] FOUND: ${item.name}`]);
          }
        } catch (e) {
